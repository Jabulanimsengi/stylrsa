...
export async function generateMetadata({
  ...
  const metadataFields = buildKeywordLandingMetadata({ keyword, province });
  ...
  try {
    ...
    if (cachedPage) {
      return {
        ...
        robots: buildSeoLandingRobots(cachedPage, 'province'),
        alternates: {
  ...
  if (localPage) {
    return {
      title: metadataFields.title,
      description: metadataFields.description,
      robots: buildSeoLandingRobots(localPage, 'province'),
      alternates: {
        canonical: canonicalUrl,
      },
      openGraph: {
...
export default async function KeywordProvincePage({ params }: PageProps) {
  ...
  return (
    <CachedSeoLandingPage
      ...
      locationFallback={province}
      breadcrumbs={[
        { label: 'Home', url: '/' },
        { label: pageData.keyword?.keyword || keyword, url: `/${keyword}` },
        { label: pageData.location?.name || province, url: `/${keyword}/${province}` },
      ]}
    />
  );
}
