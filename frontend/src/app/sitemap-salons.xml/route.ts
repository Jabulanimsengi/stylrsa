...
export async function GET() {
  ...
  try {
    const response = await fetch(`${BACKEND_URL}/seo/sitemap-salons`, {
      cache: 'no-store',
    });

    if (!response.ok) {
      throw new Error('Failed to fetch salons sitemap');
    }
    ...
    console.warn('Salons sitemap backend unavailable, using minimal fallback:', toErrorMessage(error));
    return new NextResponse(buildMinimalUrlsetXml(), {
      status: 200,
      headers: {
        'Content-Type': 'application/xml; charset=utf-8',
